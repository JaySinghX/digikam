...
#include <QTextEdit>
#include <QDateTime> //New Include
#include <QDateTimeEdit> //New Include
#include <QRadioButton> //New Include
...
#include "dmetadata.h"  //New Include
...
public:
    ...
    PhotoInfoContainer   metaData;  //New
    QDateTime            timeDate;  //New
    QDateTimeEdit*       dateTimeEdit;//New
};
      ...
      d(new Private)
{
    ...
    //Adding MetaData In the InsertTextTool


    QLabel* const exifData = new QLabel(i18nc("EXIF date/time", "EXIF Date/Time Stamp:"));
    d->metaData = DMetadata(d->previewWidget->imageIface()->originalMetadata()).getPhotographInformation();
    d->timeDate = d->metaData.dateTime;
    d->dateTimeEdit = new QDateTimeEdit(d->timeDate);
    d->dateTimeEdit->setWhatsThis(i18n("Add the Date/Time Stamp On Your Picture Using EXIF DATETIME STAMP"));
    ...
    mainLayout->addWidget(exifData,                5, 0, 1,  1); //NEW LABBEL
    mainLayout->addWidget(d->dateTimeEdit,         5, 1, 1,  1); //NEW LABEL
    mainLayout->addWidget(label1,                  6, 0, 1,  1); //OLDVAL 5
    mainLayout->addWidget(d->textRotation,         6, 1, 1,  1); //OLDVAL 5
    mainLayout->addWidget(label2,                  7, 0, 1,  1); //OLDVAL 6
    mainLayout->addWidget(d->fontColorButton,      7, 1, 1,  1); //OLDVAL 6
    mainLayout->addWidget(label3,                  8, 0, 1,  1); //OLDVAL 7
    mainLayout->addWidget(d->textOpacity,          8, 1, 1,  1); //OLDVAL 7
    mainLayout->addWidget(d->borderText,           9, 0, 1, -1); //OLDVAL 8
    mainLayout->addWidget(d->transparentText,      10, 0, 1, -1); //OLDVAL 9
